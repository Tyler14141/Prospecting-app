...
.ld-empty-state .ld-empty-icon{font-size:42px;margin-bottom:8px;opacity:.6}
.ld-empty-state h3{font-size:18px;color:var(--text);margin:0 0 8px;font-weight:700}
.ld-empty-state p{font-size:13px;color:var(--text-soft);max-width:580px;margin:0 auto 12px;line-height:1.55}
.ld-empty-state code{background:transparent;padding:0;font-size:12px;color:var(--text);font-family:ui-monospace,Menlo,Consolas,monospace}
.ld-cmd-block{display:inline-block;text-align:left;background:#0f172a;color:#e5e7eb;border-radius:8px;padding:12px 14px;margin:6px auto 0;min-width:420px;max-width:560px;font-family:ui-monospace,Menlo,Consolas,monospace}
.ld-cmd-block code{color:#e5e7eb !important}
.ld-cmd-row{display:flex;justify-content:space-between;align-items:center;gap:8px;padding:3px 0}
.ld-cmd-row:not(:last-child){border-bottom:1px solid rgba(255,255,255,0.08)}
.ld-copy-btn{background:transparent;border:1px solid rgba(255,255,255,0.2);color:#e5e7eb;padding:2px 8px;border-radius:4px;font-size:11px;cursor:pointer;transition:.15s}
.ld-copy-btn:hover{background:rgba(255,255,255,0.1)}
.ld-copy-btn.copied{background:var(--green);color:#fff;border-color:var(--green)}
  ...
  <div class="grid" style="grid-template-columns:300px 1fr;gap:18px">
    ...
    <div class="card" style="position:sticky;top:18px;align-self:start">
      ...
      <div class="ld-form">
        ...
        <button class="btn active" id="ldRun" style="width:100%;font-weight:700;padding:10px;font-size:14px">🔍 Search</button>
        <div id="ldSearchStatus" style="margin-top:8px;font-size:11px;color:var(--muted);text-align:center"></div>
...
function renderLeadEmptyState() {
  const cmds = [
    ...
    "bash refresh_after_scan.sh",
  ];
  const cmdHtml = cmds.map((c, i) =>
    `<div class="ld-cmd-row">
       <code>${c}</code>
       <button class="ld-copy-btn" data-cmd="${c.replace(/"/g, '&quot;')}" title="Copy">📋</button>
     </div>`).join("");
  return `<div class="ld-empty-state">
    <div class="ld-empty-icon">🔍</div>
    <h3>No real leads loaded yet</h3>
    <p>The Lead Discovery feed only shows leads from <strong>real data sources</strong> — no demo data appears here. To populate this feed with verified buying-intent signals from council meeting minutes across your target states, run the state-dive locally:</p>
    <div class="ld-cmd-block">${cmdHtml}</div>
    ...
    <p style="margin-top:10px;font-size:11px;color:var(--muted)">Note: this static HTML dashboard can't run the scraper itself — it needs Python + internet access to municipal websites, which a browser tab cannot do.</p>
  </div>`;
}
...
function renderLeadDiscovery() {
  ...
  // Update search-status line under the Search button
  const statusEl = document.getElementById("ldSearchStatus");
  if (statusEl) {
    if (LEADS.leads.length === 0) {
      statusEl.innerHTML = '<span style="color:#b45309">⚠ No harvested data — run state dive (see results panel)</span>';
    } else {
      statusEl.innerHTML = `Found <strong style="color:var(--accent)">${visible.length}</strong> of ${LEADS.leads.length} real leads`;
    }
  }
  ...
  if (visible.length === 0) {
    if (LEADS.leads.length === 0) {
      ...
      c.querySelectorAll(".ld-copy-btn").forEach(btn => {
